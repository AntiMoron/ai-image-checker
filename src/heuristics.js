const { sampleDctBlocks } = require('./dct');
...
function coefficientStats(blocks) {
  return MID_FREQUENCIES.map((index) => {
    const values = blocks.map((block) => Math.abs(block[index]));
    const avg = mean(values);
    const spread = stddev(values, avg);
    const regularity = avg <= 0.0001 ? 0 : clamp01(1 - spread / (avg + 0.0001));
    return { index, avg, spread, regularity };
  });
}
...
function analyzeFrequencyHeuristics(grayImage) {
  const blocks = sampleDctBlocks(grayImage);
  const notes = [
    'Frequency analysis is heuristic and is not proof of SynthID or any specific watermark system.'
  ];
  ...
  const stats = coefficientStats(blocks);
